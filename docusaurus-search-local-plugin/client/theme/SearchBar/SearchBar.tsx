...
export default function SearchBar(): ReactElement {
  ...
  const [hits, setHits] = useState<SearchResults>({ titles: [], headers: [], content: [] });
  ...
  const onInputChange = (evt: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if(indexes && evt.target.value) {
        ...
        const res = getSearchResults(indexes[0].index, evt.target.value)
          .map(r => indexes[0].documents.find(doc => doc.i === r.ref));
      
        const te = res.slice(0, 3);

        const headings = indexes[1].index.search(query)
        .map(r => indexes[1].documents.find(doc => doc.i === r.ref));

        const content = indexes[2].index.search(query)
        ...
        {options.length > 0 && (
          <ol>
            {options.map((hit) => {
              ...
              return (
                <li key={hit.key}>
                  <a onClick={() => onSelected(hit.url, hit.text)}>{hit.text}</a>
                  <br />
                  <span>{type()}</span>
                </li>
              )
