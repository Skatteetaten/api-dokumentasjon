...
const getHeadingAndContentDocuments = (
  sections,
  pageTitle,
  url,
  titleId
  ...
  return sections.reduce((prev, { content, title, hash }) => {
    const headingDocuments = title !== pageTitle
...
module.exports.scanDocuments = async function scanDocuments(
  ...
  const initalValue = { titleDocuments: [], headingDocuments: [], contentDocuments: [] };
  return DocInfoWithFilePathList.reduce(async (promise, { filePath, url, type }) => {
    ...
    const t = getHeadingAndContentDocuments(sections, pageTitle, url, titleId);
